...
export default function TabOneScreen() {
  ...
  const [selectedFirebaseIngredients, setSelectedFirebaseIngredients] =
    useState<Item[]>([])
  const firebaseCategories: Item[] = useFirebaseCategories()
  const [selectedFirebaseCategories, setSelectedFirebaseCategories] = useState<
    Item[]
  >([])

  const [searchMode, setSearchMode] = useState<'database' | 'cookbook'>(
    'database'
  )
  ...
  useEffect(() => {
    if (searchMode === 'database') {
      setSelectedApiIngredients(
        apiIngredients.map((ingredient) => ({ ...ingredient, selected: false }))
      )
    } else if (searchMode === 'cookbook') {
      setSelectedFirebaseIngredients(
        firebaseIngredients.map((ingredient) => ({
          ...ingredient,
          selected: false,
        }))
      )
  ...
  useEffect(() => {
    if (searchMode === 'database') {
      setSelectedApiCategories(
        apiCategories.map((category) => ({ ...category, selected: false }))
      )
    } else if (searchMode === 'cookbook') {
      setSelectedFirebaseCategories(
        firebaseCategories.map((category) => ({ ...category, selected: false }))
      )
    }
  ...
  const handleSearchInFirebase = async () => {
    ...
    const matchingRecipeId = await searchRecipesInFirebase(
      selectedCookBookIngredients,
      selectedCookBookCategories,
      userID
    )
    ...
    if (matchingRecipeId) {
      router.push({
        ...
        params: {
          recipeID: matchingRecipeId.toString(),
          originScreen: 'index',
        },
      })
    } else {
  ...
  const getMeal = async () => {
    ...
    const mealId = await fetchMeal(
      ItemListToCSVString(selectedApiIngredients),
      ItemListToCSVString(selectedApiCategories)
    )
    ...
    <View style={gStyles.screenContainer}>
      <SearchSwitch
        onToggle={(isDatabaseSearch) =>
          setSearchMode(isDatabaseSearch ? 'database' : 'cookbook')
        }
      />
      <ScrollView
      ...
      <Pressable
        onPress={() =>
          searchMode === 'cookbook' ? handleSearchInFirebase() : getMeal()
        }
        style={({ pressed }) => [
          gStyles.squareButtonText,
          {
