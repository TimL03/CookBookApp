...
export default function AddRecipeScreen(this: any) {
  ...
  if (params.recipeID) {
    useEffect(() => {
      const fetchRecipe = async () => {
        const fetchedRecipe = await getRecipeById(
          userID.toString(),
          params.recipeID.toString()
        )
  ...
  const [galeriePermissionAlertVisible, setGaleriePermissionAlertVisible] =
    useState(false)
  const [cameraPermissionAlertVisible, setCameraPermissionAlertVisible] =
    useState(false)

  // Initialize and set state variables for multi-line input heights
  const [multiLineHeights, setMultiLineHeights] = useState(
    new Array(steps.length).fill(80)
  )
  ...
  return (
    <>
      ...
      >
        ...
        <ScrollView
          style={[
            gStyles.fullScreenBackgroundContainer,
        ...
        >
          ...
          {imageUri != null ? (
            <Pressable onPress={() => setChooseImageAlertVisible(true)}>
              <Image style={gStyles.image} source={{ uri: imageUri }} />
            </Pressable>
          ) : (
            <Pressable
              onPress={() => setChooseImageAlertVisible(true)}
              style={({ pressed }) => [
                styles.addImage,
                {
                  backgroundColor: pressed
              ...
              {categories.map((category, index) => (
                <View key={index} style={gStyles.cardInput}>
                  <TextInput
                    ...
                    blurOnSubmit={true}
                    onSubmitEditing={() => {
                      this.categorieInputs[index + 1] != undefined
                        ? this.categorieInputs[index + 1].focus()
                        : null
                    }}
                    ref={(input) => {
                      this.categorieInputs[index] = input
                    }}
                    placeholderTextColor={Colors.dark.text}
                    onChangeText={(text) => {
                  ...
                  <Pressable
                    ...
                    style={[gStyles.iconButton, styles.marginRight]}
                  >
                    <X
                      color={Colors.dark.text}
                      size={22}
            ...
            <View style={styles.gap}>
              {ingredients.map((ingredient, index) => (
                <View
                  key={index}
                  style={[
                    gStyles.HorizontalLayout,
                    { gap: 12, zIndex: activeDropdown === index ? 1 : 0 },
                  ]}
                >
                  <View style={[gStyles.cardInput, { flex: 1 }]}>
                    <TextInput
                      ...
                      blurOnSubmit={false}
                      onSubmitEditing={() => {
                        this.dropDownInputs[index].focus()
                      }}
                      ref={(input) => {
                        this.ingredientInputs[index] = input
                      }}
