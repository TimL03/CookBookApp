    ...
    try {
      if (!selectedIngredients && !selectedCategories) {
        ...
        if (selectedIngredients) {
          const response = await fetch(
            `https://www.themealdb.com/api/json/v2/9973533/filter.php?i=${selectedIngredients}`
          )
          const data = await response.json()
        ...
        if (selectedCategories) {
          const response = await fetch(
            `https://www.themealdb.com/api/json/v2/9973533/filter.php?c=${selectedCategories}`
          )
          const data = await response.json()
        ...
        if (matchedMeals.length > 0) {
          const randomMealId =
            matchedMeals[Math.floor(Math.random() * matchedMeals.length)].idMeal
          setSelectedMealId(randomMealId)
          return randomMealId
        } else {
...
export const useGetMealById = (mealId: string) => {
  ...
  const fetchMeal = async () => {
    try {
      ...
      const response = await fetch(
        `https://www.themealdb.com/api/json/v2/9973533/lookup.php?i=${mealId}`
      )
      const data = await response.json()
